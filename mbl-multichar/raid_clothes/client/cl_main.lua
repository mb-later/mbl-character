...
Citizen.CreateThread(function()
    while RLCore == nil do
        Citizen.Wait(0)
        TriggerEvent("RLCore:GetObject", function(yarra) RLCore = yarra end)
    end
end)
...
AddEventHandler('pw_character:client:setupCharCreation', function(selectionCoords)   
...
Citizen.CreateThread(function()
    while true do
        ...
        if statusS == true then
            TriggerServerEvent("mbl-spawn:inspawn", true)
        else
            Citizen.Wait(750)
        end
    end
...
function Save(save, close)
    ...
    if save then
        ...
        if not startingMenu or passedClothing then
            ...
            TriggerEvent("mbl-spawn:finishedClothing","Finished")
    ...
    if close then
        ...
        if #(entity - vector3(408.78, -998.57, -99.99)) <  70 then
            DoScreenFadeOut(1500)
            TriggerEvent("mbl-kashacters:selectnation")
            TriggerServerEvent("CloseCharCreLoc")
        end
    end
...
AddEventHandler('raid_clothes:ListOutfits', function(skincheck)
  ...
  TriggerEvent('mbl-ui:ShowUI', 'show', menuData)
end)
...
RegisterCommand("camfix", function ()
    FreezePedCameraRotation(PlayerPedId(), false)
end)
...
AddEventHandler("mbl-polyzone:enter", function(zone, data)
    local currentZone = MenuData[zone]
    if currentZone then
        TriggerEvent('mbl-ui:ShowUI', 'show', ("[M] %s"):format(currentZone.text))
        TriggerEvent("raid_clothes:enable", true)
        listenForKeypress(zone, currentZone, ((data and data.isFree) and true or false))
    end
end)

AddEventHandler("mbl-polyzone:exit", function(zone)
    local currentZone = MenuData[zone]
    if currentZone then
        listening = false
        TriggerEvent('mbl-ui:HideUI')
...
AddEventHandler('clothing:blips', function()
   ...
   for _, item in pairs(Clothing) do
        if not showClothes then
            ...
            SetBlipAsShortRange(item.blip, true)
            BeginTextCommandSetBlipName("STRING")
            AddTextComponentString("Kıyafet Mağazası")
            EndTextCommandSetBlipName(item.blip)
        end
    end
end)
