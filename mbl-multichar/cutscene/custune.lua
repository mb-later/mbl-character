RegisterCommand("seçimekranı", function()
    TriggerEvent("mbl-kashacters:selectnation")
end)
RegisterNetEvent('print')
AddEventHandler('print', function()
    TriggerEvent("setentityfirstlogin")
end)


RegisterNetEvent("setentityfirstlogin")
AddEventHandler("setentityfirstlogin", function()
    DoScreenFadeOut(1000)
    Wait(900)
    FreezeEntityPosition(PlayerPedId(), false)
    SetEntityCoords(PlayerPedId(), -14.49327, -1437.79, 31.015558)
    SetEntityHeading(PlayerPedId(), 180.75903)
    DoScreenFadeOut(1000)
    TaskGoToCoordAnyMeans(PlayerPedId(), -14.02836, -1448.873, 30.646337, 1.0, 0, 0, 786603, 1.0)
    Wait(100)
    DoScreenFadeIn(150)
end)
...
RegisterNetEvent('mbl-kashacters:selectnation')
AddEventHandler('mbl-kashacters:selectnation', function()
    local menu = {
        { ['label'] = 'Bu şehirde yaşıyorum.', ['action'] = 'print', ['value'] = nil, ['triggertype'] = 'client', ['color'] = ('dark' or 'dark')}
    }
  
    table.insert(menu, { ['label'] = ("Yurt dışından geldim."), ['action'] = ('introCinematic:start'), ['value'] = nil, ['triggertype'] = 'client', ['color'] = ('success')})
    TriggerEvent('mbl_interact:generateMenu', menu, "Lütfen bir seçim yapınız.")
end)
