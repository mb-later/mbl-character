...
function setColor(){
    ...
    if(autoPreview === true) {
        if(previewTrigger !== null && previewTriggerType !== null) {
            $.post("http://mbl_interact/colorPreview", JSON.stringify({
                r: parseInt(r.value),
                g: parseInt(g.value),
                b: parseInt(b.value),    
...
function populateForm(form) {
    ...
    if(form.opt !== undefined) {
        if(form.opt.allowCoords !== undefined && form.opt.allowCoords === true) {
            $('#jsonCopy').html(form.json);
            $('#stndCopy').html(parseFloat(form.coords.x) + ', ' + parseFloat(form.coords.y) + ', ' + parseFloat(form.coords.z) +', '+ parseFloat(form.coords.h))
            $('#tableCopy').html("table = { ['x'] = " + parseFloat(form.coords.x) + ", ['y'] = " + parseFloat(form.coords.y) + ", ['z'] = " + parseFloat(form.coords.z) + ", ['h'] = " + parseFloat(form.coords.h) + " }")
        ...
        if (index == 0) {
            if(option.infoHtml !== undefined && option.infoHtml !== null) {
                if(option.infoHtml.type == "tooltip") {
                    $('#selection').append('<li class="nav-item"><a class="nav-link active btn btn-secondary" style="min-width:195px; max-width:195px;" title="' + option.infoHtml.title + '" id="navID-' + index + '" data-idx="' + index + '" data-tooltip="tooltip" data-html="true" data-toggle="tab" data-trigger="hover" data-placement="bottom" href="#home" role="tab" aria-controls="home" aria-selected="true" data-act="selectActionSlider"><small>' + option.label + ' <i class="fas fa-search"></i></small></a></li>');
                } else {
                    $('#selection').append('<li class="nav-item"><a class="nav-link active btn btn-secondary" style="min-width:195px; max-width:195px;" title="' + option.infoHtml.title + '" id="navID-' + index + '" data-idx="' + index + '" data-popover="popover" data-html="true" data-toggle="tab" data-trigger="hover" data-placement="bottom" data-content="' + option.infoHtml.content +'" href="#home" role="tab" aria-controls="home" aria-selected="true" data-act="selectActionSlider"><small>' + option.label + ' <i class="fas fa-search"></i></small></a></li>');
                }
                $('[data-popover="popover"]').popover();
                $('[data-tooltip="tooltip"]').tooltip();
            } else {
                $('#selection').append('<li class="nav-item"><a class="nav-link active btn btn-secondary" style="min-width:195px; max-width:195px;" id="navID-' + index + '" data-idx="' + index + '" data-toggle="tab" href="#home" role="tab" aria-controls="home" aria-selected="true" data-act="selectActionSlider"><small>' + option.label + ' <i class="fas fa-search"></i></small></a></li>');
            }
        } else {
            if(option.infoHtml !== undefined && option.infoHtml !== null) {
                if(option.infoHtml.type == "tooltip") {
                    $('#selection').append('<li class="nav-item"><a class="nav-link btn btn-secondary" style="min-width:195px; max-width:195px;" id="navID-' + index + '" title="' + option.infoHtml.title + '" data-idx="' + index + '" data-toggle="tab" data-tooltip="tooltip" data-html="true" data-trigger="hover" data-placement="bottom" href="#home" role="tab" aria-controls="home" aria-selected="true" data-act="selectActionSlider"><small>' + option.label + ' <i class="fas fa-search"></i></small></a></li>');        
                } else {
                    $('#selection').append('<li class="nav-item"><a class="nav-link btn btn-secondary" style="min-width:195px; max-width:195px;" id="navID-' + index + '" title="' + option.infoHtml.title + '" data-idx="' + index + '" data-toggle="tab" data-popover="popover" data-html="true" data-trigger="hover" data-placement="bottom" data-content="' + option.infoHtml.content +'" href="#home" role="tab" aria-controls="home" aria-selected="true" data-act="selectActionSlider"><small>' + option.label + ' <i class="fas fa-search"></i></small></a></li>');        
                }
                $('[data-popover="popover"]').popover();
                $('[data-tooltip="tooltip"]').tooltip();
            } else {
                $('#selection').append('<li class="nav-item"><a class="nav-link btn btn-secondary" style="min-width:195px; max-width:195px;" id="navID-' + index + '" data-idx="' + index + '" data-toggle="tab" href="#home" role="tab" aria-controls="home" aria-selected="true" data-act="selectActionSlider"><small>' + option.label + ' <i class="fas fa-search"></i></small></a></li>');        
            }
        }
...
function closeMenu() {
    ...
    $.post("http://mbl_interact/NUIFocusOff", JSON.stringify({}));
    $('#formButtons').css({"display":"block"});
...
$( function() {
    $(document).on("keydown", function (key) {
        if (Config.closeKeys.includes(key.which)) {
            if(preventClose === false) {
                if (cancelEvent !== undefined && cancelEvent !== null) {
                    $.post("http://mbl_interact/doCancel", JSON.stringify({
                        post: cancelEvent,
                    }));
                    cancelEvent = null
                }
        ...
        if(key.which == 90 && enableKeySwitch == true) {
            ...
            $.post("http://mbl_interact/toggleNUI", JSON.stringify({}));
        }
        if(key.which == 67 && swapKeyEnabled == true) {
            $.post("http://mbl_core/rotateCharacter", JSON.stringify({}));
        }
        if(key.which == 88 && taskHandsUp == true) {
            $.post("http://mbl_core/handsUpCharacter", JSON.stringify({}));
        }
        if(key.which == 90 && resetCamera !== null) {
            $.post("http://mbl_core/resetCamera", JSON.stringify({
                cameraPos: resetCamera
            }));
        }
        if(key.which == 90 && allowSkinMouseMove === true) {
            $.post("http://mbl_interact/turnOffNUI", JSON.stringify({ }));
    ...
    $(document).on('change', '[data-skin=true]',function(){
        value = $(this).val();
        data = $(this).data('data');
        $.post("http://mbl_base/skinSlider", JSON.stringify({
            data: data,
            value: value,
        }));
    });
    ...
    $(document).on('click','[data-act=blipColorSelect]',function(){
        ...
        $.post("http://mbl_interact/selectBlipColor", JSON.stringify({
            color: colorId,
    ...
    $(document).on('click','[data-act=submitForm]',function(){
        ...
        $.post("http://mbl_interact/submitForm", JSON.stringify({
            action: action,
            trigger: sendType,
            response: response
        }));
    ...
    $(document).on('click','[data-act=blipSelected]',function(){
        ...
        if (blipId == null || blipId == "" || blipId < 0 || blipId == undefined) {
            ...
            $.post("http://mbl_interact/selectBlip", JSON.stringify({
            blip: blipId,
            trigger: trigger,
    ...
    $(document).on('click','#colorSave',function(){
        ...
        if(!$(this).hasClass('disabled')) {
            $.post("http://mbl_interact/colorSave", JSON.stringify({
                r: parseInt(r),
                g: parseInt(g),
                b: parseInt(b),    
                action: trigger,
    ...
    $(document).on('click','[data-act=completeRequest]',function(){
        if(!$(this).hasClass('disabled')) {
            ...
            if (action == "undefined" || action == "") {
                ...
                $.post("http://mbl_interact/requestAction", JSON.stringify({
                    action: action,
                    trigger: trigger,
                    value: value
                }));
    ...
    $(document).on('click','[data-act=closeMenu]',function(){
        ...
        $.post("http://mbl_interact/cancelAction", JSON.stringify({
            action: action,
            trigger: sendType,
            response: response
        }));
    ...
    $(document).on('click','[data-act=skinBack]',function(){
        ...
        var data = $(this).data('data');
        $.post("http://mbl_interact/doSkinReturn", JSON.stringify({
            trigger: trigger,
            triggertype: triggertype,
            data: data
        }));
    ...
    $(document).on('click','[data-act=selectActionSlider]',function(){
        ...
        $.post("http://mbl_interact/sliderAction", JSON.stringify({
            action: action,
            trigger: method,
            response: associate
        }));
    ...
    $(document).on('click', '[data-act=selectActionSliderSkin]', function() {
        ...
        $.post("http://mbl_interact/sliderActionSkin", JSON.stringify({
            trigger: trigger,
            triggertype: triggertype,
            data: data,
        }));
