...
function BuildLocations(data)
    ...
    if data then
        if data.position and not data.jail then
            ...
            results[1] = { name = "last", label = "Son Konum", coords = coords, rot = rot, spawn = vector4(data.position.x, data.position.y, data.position.z, 100) };
        end
...
RegisterNetEvent("nvd-select:set")
AddEventHandler("nvd-select:set" , function()
    BuildSelector(RLCore.Functions.GetPlayerData());
end)